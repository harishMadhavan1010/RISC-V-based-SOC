...
\TLV
   ...
   // Load-Store:
   m4_asm(SW, r0, r10, 100)             // Store the value in the Data Memory
   m4_asm(LW, r15, r0, 100)             // Load the stored value in the x15 of RegFile
   m4_asm(SW, r1, r10, 1000)            // To verify if load instruction works (TEST)
   m4_asm(SW, r2, r10, 1100)            // To verify if load instruction works (TEST)
   ...
   |cpu
      ...
      @3
         ...
         $result[31:0] = $is_add ? $src2_value + $src1_value :
                         $is_sub ? $src2_value - $src1_value :
         ...
         // Register File Write
         $dest_not_zero = ! $rd ==? 5'b00000;
         $write_valid = $rd_valid & $dest_not_zero & $valid;
         ...
         $valid_jump = ($is_jal | $is_jalr) & $valid;
